...
void GetScore(struct Score grade[], int n)
{
    for (int i = 0; i < n; i++)
    {
        scanf_s("%d%s", &grade[i].id, &grade[i].name);
        for (int j = 0; j < 17; j++)
            scanf_s("%d", &grade[i].score[j]);
    }
}
void CalcuScore(struct Score grade[], int n)
{
...
void Ranking(struct Score grade[], int n)     //排序
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        for(int j=0;j<n;j++)
        {
            if(j!=i&&grade[i].total<grade[j].total)      //看有哪些比自己低，避免并列问题，也不用排序交换
                grade[i].rank++;
